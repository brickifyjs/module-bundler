...
function run(input, output) {
	try {
		...
		var process = function (filepath, data) {
			data.replace(declaration, function (match, binding) {
				bindings[binding] = 0
			})
		...
		var exportCode = function (filename, filepath, def, variable, eq, rest, uuid) {
			...
			var string = /(["'])((?:\\\1|.)*?)(\1)/g
			var candidates = Object.keys(bindings).map(function (binding) {
				return binding + (bindings[binding] - 1)
			}).join("|")
			code = code.replace(string, function (match, open, data, close) {
				var variables = new RegExp(Object.keys(bindings).map(function (binding) {
					return binding + (bindings[binding] - 1)
				}).join("|"), "g")
